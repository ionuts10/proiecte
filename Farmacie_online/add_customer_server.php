...
if(empty($nume) || empty($prenume) || empty($telefon) || empty($abonat_newsletter) || empty($adresa_facturare) || empty($adresa) || empty($oras) || empty($judet) || empty($data_nasterii) || empty($sex)){
...
if(count($erori) == 0){
	...
	$query_inregistrare = "INSERT INTO tabela_clienti (nume, prenume, data_nasterii, sex, email, telefon, judet, oras, adresa, adresa_facturare, abonat_newsletter) VALUES ('$nume', '$prenume', '$data_nasterii', '$sex', '$email', '$telefon', '$judet', '$oras', '$adresa', '$adresa_facturare', '$abonat_newsletter')";
